...
def run_minimal_chain_task_gpu(alg, integ, eps):
    ...
    if alg == 'HMC':
        ...
        for L in range(1, MAX_HMC_STEPS + 1):
            ...
            # Skip SBC and PPC if any basic metrics failed
            if failures:
                continue

            # SBC
            logger.info(f"  Running SBC check for {alg}-{integ} with L={L}")
            ...
            # PPC
            logger.info(f"  Running PPC check for {alg}-{integ} with L={L}")
            ppc_res = ppc_gpu(
        ...
        for L in range(1, MAX_HMC_STEPS + 1):
            logger.info(f"  Trying L={L} for {alg}-{integ}")
            # use your GPU "seeker" to find minimal sweeps
            ...
            # record if passed
            if not failures:
                logger.info(f"  SUCCESS: {alg}-{integ} with L={L}, sweeps={sweeps}")
                # keep the smallest sweeps
        ...
        for L in range(1, MAX_HMC_STEPS + 1):
            logger.info(f"  Trying L={L} for {alg}-{integ}")
            # use your GPU "seeker" to find minimal particles
            ...
            # record if passed
            if not failures:
                logger.info(f"  SUCCESS: {alg}-{integ} with L={L}, particles={Np}")
                # keep the smallest number of particles
    ...
    logger.info(f"======= COMPLETED TASK: {alg} with {integ} =======")
